...
public class RaylibInput : IInputProvider
{
    ...
    {
        var ctrlDown = Raylib_cs.Raylib.IsKeyDown(Raylib_cs.KeyboardKey.LeftControl) ||
        ...
        long millisecondsSinceEpoch = DateTimeOffset.UtcNow.ToUnixTimeMilliseconds();
        {
            foreach (var value in Enum.GetValues(typeof(TextInputCommands)))
            {
                ...
                if (Raylib_cs.Raylib.IsKeyDown(key))
                {
                    if (msPassed > 0)
                    {
                        _timeToAllowNextInputCommand[(int)value] =
                            (millisecondsSinceEpoch + (msPassed >= 250 ? 450 : 45));
                        var command = (TextInputCommands)value;
                        if ((command == TextInputCommands.MoveCaretEnd) && !ctrlDown)
                        {
                            continue;
                        }

                        if ((command == TextInputCommands.MoveCaretEndOfLine) && ctrlDown)
                        {
                            continue;
                        }

                        if ((command == TextInputCommands.MoveCaretStart) && !ctrlDown)
                        {
                            continue;
                        }

                        if ((command == TextInputCommands.MoveCaretStartOfLine) && ctrlDown)
                        {
                            continue;
                        }
    ...
    public KeyboardInteractions? GetKeyboardInteraction()
    {
        ...
        if (Raylib_cs.Raylib.IsKeyDown(Raylib_cs.KeyboardKey.Space) ||
            Raylib_cs.Raylib.IsKeyDown(Raylib_cs.KeyboardKey.Enter))
        {
