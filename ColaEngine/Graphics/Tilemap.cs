...
public class Tilemap
{
    ...
    public void Draw()
    {
        for (int i = 0; i < Count; i++)
        {
            ...
            tile.Draw(new Rectangle(position, tile.Width * Scale.X, tile.Height * Scale.Y), 
                new Vector2(0,0), 
                0f, 
                Color.White);
        }
    ...
    /// </summary>
    /// <param name="filename">The path to the xml file, relative to the resources root directory.</param>
    ...
    public static Tilemap FromFile(string filename)
    {
        using (Stream stream = File.OpenRead(filename))
        {
            using (XmlReader reader = XmlReader.Create(stream))
            {
                ...
                // Load the texture directly via Raylib, same as Game.LoadContent()
                Texture2D texture = Raylib.LoadTexture(contentPath);

                // Build the region using YOUR TextureRegion(Texture2D, Rectangle) ctor
                TextureRegion textureRegion = new TextureRegion(texture, new Rectangle(x, y, width, height));

                // Your Tileset ctor derives Columns/Rows internally from the region size
                Tileset tileset = new Tileset(textureRegion, tileWidth, tileHeight);

                // <Tiles> — each line is a row, space-separated tileset indices per column
